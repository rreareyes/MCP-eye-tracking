...
folders.root                    = pwd;
folders.data                    = fullfile(folders.root, 'testdata'); % folder in which data is stored (each folder in folders.data is considered 1 subject)
folders.output                  = fullfile(folders.root, 'processed'); % folder for output (will use structure in folders.data for saving output)
...
for e = 1:nfold
    ...
    for f = 1:nfile
        ...
        [data.time, data.left.X, data.left.Y, data.right.X, data.right.Y] = importRED250(fullfile(folders.data,fold(e).name,file(f).name), 33, [opt.xres opt.yres], opt.missingx, opt.missingy);
