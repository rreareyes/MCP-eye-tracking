...
bayesianModelData.indCueModel{18} = [1]; % Tallying
bayesianModelData.indCueModel{19} = [1]; % Serial
        ...
        for iModel = 1 : bayesianModelData.nModel
            ...
            elseif ismember(iModel, tallying_4)
                ...
                for iTrial = 1 : size(logRegData.Regressors{iSubject, iPhase}, 1)
                    ...
                    bayesianModelData.xModel{iSubject, iModel, iPhase}(iTrial) = sum(logRegData.Regressors{iSubject, iPhase}(iTrial, 1:3));
            ...
            elseif ismember(iModel, tallying_2)
                ...
                tempi = logRegData.Regressors{iSubject, iPhase}.* [8, 4, 2, 1];
                
                for iTrial = 1 : size(logRegData.Regressors{iSubject, iPhase}, 1)
                    
                    bayesianModelData.xModel{iSubject, iModel, iPhase}(iTrial, 1) = sum(tempi(iTrial, :));
