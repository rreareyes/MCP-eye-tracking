...
cd('..')
folders.root    = pwd;
folders.data    = fullfile(folders.root, 'Eye_tracking', 'Subject_files'); % folder in which data is stored (each folder in folders.data is considered 1 subject)
folders.output  = fullfile(folders.root, 'Eye_tracking', 'Results'); % folder for output (will use structure in folders.data for saving output)
folders.scripts = fullfile(folders.root, 'Scripts');
folders.I2MC    = 'C:/Users/rerr_/Google Drive/Graduate/Lab/Studies/MyFunctions/I2MC-release';

addpath(genpath(folders.I2MC))
addpath(folders.scripts)
...
opt.downsampFilter = false; % use chebychev filter when downsampling? 1: yes, 0: no. requires signal processing toolbox. is what matlab's downsampling functions do, but could cause trouble (ringing) with the hard edges in eye-movement data
% 
...
fid = fopen(fullfile(folders.output, 'fixation_data.txt'),'w');
...
for e = 1:nfold
    ...
    for f = 1:nfile
        ...
        [data.time, data.left.X, data.left.Y, data.right.X, data.right.Y] = Ba_importRED250(fullfile(folders.data,fold(e).name,file(f).name), 1, [opt.xres opt.yres], opt.missingx, opt.missingy);
