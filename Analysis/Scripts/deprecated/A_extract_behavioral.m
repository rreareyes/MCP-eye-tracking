...
file.Output = fullfile(folder.Results, 'behavior_dataset.xls');
file.Stim   = fullfile(folder.Root, 'Behavior', 'stim_eyetracker.mat');
file.Sequence = fullfile(folder.Root, 'Behavior', 'normal_sequence.mat');

%% Get number of files per phase
file.AllList = dir(fullfile(folder.Behavior, '*.mat'));
...
for iSubject = 1:nSubjects
    ...
    for iPhase = 1:nPhases
        ...
        subject.ParametersFile = dir(fullfile(folder.Behavior, [subject.Number '_expParameters_' subject.Phase '.mat']));
        subject.ParametersPath = fullfile(folder.Behavior, subject.ParametersFile.name);

        load(subject.ParametersPath, 'param');

        subject.BlockList = dir(fullfile(folder.Behavior, [subject.Number '_' subject.Phase '_Block*.mat']));
        ...
        for iBlock = 1:nBlocks
            ...
            % Get file to load
            subject.BlockFile = fullfile(folder.Behavior, subject.BlockList(iBlock).name);
...
%             subject.StatesRight(subject.StatesRight == 2) = 0;
    ...
    subject.DataSet = [subject.ID, subject.PhaseID, subject.BlockID, subject.PhaseSequence, subject.Environment, subject.PhaseResponses, subject.PhaseRT, subject.PhaseRank, subject.PhaseStates];
...
data.VariableNames = {'id', 'phase', 'block', 'seq_id', ...
...
data.TidySet       = [data.VariableNames; num2cell(group.DataSet)];

xlswrite(file.Output, data.TidySet)
