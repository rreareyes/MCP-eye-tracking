...
%% Define output files
file.WinModel      = fullfile(folder.Results, 'win_model.csv');
file.WinModelPrior = fullfile(folder.Results, 'win_model_prior.csv');

%% Define paths to input dataset
file.Behavior = fullfile(folder.Results, 'responses_dataset.csv');

%% Load behavior data
data.Raw  = xlsread(file.Behavior);
data.Base = data.Raw(:, [1:3, 37, 2, 30:33, 16, 14]); % ID, experiment, phase, state 1-4, response, p_left
