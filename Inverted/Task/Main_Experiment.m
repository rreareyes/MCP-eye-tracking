...
if options.Debug == 1
    ...
    file.ExpSequence    = 'debug_sequence';
...
for iPhase = options.StartPhase:nPhases
    ...
    elseif iPhase == 3
        load(file.ExpSequence, 'stim')
        param.namePhase = {'EP'};

    end

    %% Get base parameters for stimulus on different phases
    if options.New == 1 || options.StartBlock == 1
    ...
    elseif options.New == 2 && options.StartBlock > 1
